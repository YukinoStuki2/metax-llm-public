...
- 你已经能跑通服务与评测，但想在固定的评测口径下，自动寻找“准确率达标且吞吐更高”的一组推理参数。
- 你希望脚本“不中断”：单轮启动失败/评测超时/服务崩溃都会被记录，然后继续探索下一组。
...
如果你需要通知，先准备 `tune_secrets.sh`（可选）：
...
- 它会写一个 `autotune.watchdog.pid`，方便你定位 watchdog PID。
...
脚本会自动加载 `./tune_secrets.sh`（若存在），你通常把通知/密钥/个性化开关都放这里。
...
- 如果你没有显式设置这两个变量，脚本默认开启“跳过 pip 安装/跳过模型下载”。
- 如果你显式设置了 `SKIP_PIP_INSTALL=0`（或其他值），脚本会尊重你的设置。
...
默认策略是“等待 + 重试”。当你打开 `--port_busy_kill`（或 `TUNE_PORT_BUSY_KILL=1`）时，会：
...
- 你调参看的准确率/吞吐口径是固定的（bonus 子集、batch 模式、固定 tokenizer）。
- 如果你想换 `--which` 或换 tokenizer 目录，需要改 `auto_tune.py` 的 `eval_cmd` 段。
...
- 检查 `eval_local.py` 是否仍输出类似：
  - `Accuracy (RougeL-F1 mean, RAW): ...`
  - `Throughput RAW: ... (prompt+answer)_tokens/s=...`
- 如果你改过 `eval_local.py` 的 summary 文案，需要同步更新 `auto_tune.py` 的正则。
...
另外：默认只发 `best,crashed,done`，如果你希望 `abnormal` 也发，设置：
...
- 内置 `order` 只覆盖一组常用参数。
- 你可以用外部 JSON 合并：`TUNE_SEARCH_SPACE_FILE=...`。
- 如果你增加了新的参数名：脚本会把它追加到探索顺序最后（不会影响已有顺序）。

### 4.8 为什么调参过程中会联网？

`auto_tune.py` 自身不会在请求路径做联网探测；但通知（webhook/email）属于“向外发请求”。

- 如果你运行在断网环境，请关闭通知相关配置（不要设置 webhook/smtp）。
- 推理服务本身（评测 run 阶段）仍应保持断网可运行。

### 4.9 我想换评测子集/换 tokenizer/换 eval 参数
...
- 同时确认 `parse_eval_output` 的正则仍能解析到你关心的指标。
