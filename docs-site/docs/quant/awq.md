...
- **建议**：除非你非常确定模型无需 remote code，否则不要关闭。
...
- **脚本行为**：脚本会在运行时 `os.environ.setdefault("TRANSFORMERS_NO_TORCHVISION", "1")`，即默认设为 `1`（若你已设置其它值则保留）。
...
这样做的目的：让量化校准阶段看到的 prompt 更贴近你真实推理时的格式，从而减少量化带来的分布偏移。
...
2. 环境兼容性：默认设置 `TRANSFORMERS_NO_TORCHVISION=1`。
3. 加载 tokenizer 与校准数据：
  - 抽样得到 `AWQ_NUM_CALIB` 条文本。
  - 可选包装成 chat prompt。
4. 构造 `quant_config` 并打印：
...
脚本会在导入失败时直接退出，并提示你安装量化依赖。按仓库建议在单独环境安装 `requirements-quantize-awq.txt` 后再运行。
...
- 让校准集分布更贴近你的线上题目：校准集来自同类领域文本比“泛文本”更稳。
...
脚本已经尝试显式写出 `quant_config.json`；如果你看到日志里有写文件失败的 warning，请检查：
