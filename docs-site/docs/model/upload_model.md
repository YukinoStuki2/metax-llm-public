...
| 环境变量 | 作用 | 影响范围 | 默认/备注 |
|---|---|---|---|
| `REPO_ID` | 覆盖 `--repo-id` 默认值 | 决定上传到哪个 repo | 未设置则使用脚本内置默认 |
| `MODEL_DIR` | 覆盖 `--model-dir` 默认值 | 决定上传哪个本地目录 | 未设置则使用脚本内置默认 |
| `MODELSCOPE_API_TOKEN` | 提供 token（优先） | 登录与权限 | 与 `--token` 二选一或同时存在（参数优先） |
| `MODELSCOPE_TOKEN` | 提供 token（备选） | 登录与权限 | 当 `MODELSCOPE_API_TOKEN` 未设置时使用 |
...
3. 轻量目录校验（warning 级别）：
   ...
   - 缺失时仅打印 warning：
     - 若你上传的是“完整模型目录”，这通常意味着导出目录不对
     - 若你上传的是 LoRA adapter，可忽略该 warning
4. 登录策略：
...
5. 执行上传：
   ...
   - 若失败且没 token，会额外提示你设置 `MODELSCOPE_API_TOKEN` 或使用 `--token`
...
### Q1：脚本提示 "未提供 token"，我可以忽略吗？

可以忽略，但只有在以下情况才可能成功：

- 你所在环境已经通过 modelscope 的本地配置缓存登录过；或
...
- 如果你上传的是完整模型：请检查 `--model-dir` 是否指向了正确的导出目录（例如融合/量化后的输出目录）。
- 如果你上传的是 LoRA adapter：这两个文件本来就可能不存在，可以忽略 warning。
...
如果失败时你没有提供 token，优先按提示补充 `MODELSCOPE_API_TOKEN` 再重试。

### Q4：`--repo-id` 和 `REPO_ID` 谁的优先级更高？

命令行参数优先级更高；只有当你不传 `--repo-id` 时才会读取 `REPO_ID`。
