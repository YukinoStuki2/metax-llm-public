...
- `env_force.sh`：强制导入一套“干净参数”，清除当前 shell 中遗留的环境变量污染
...
- 只在“未设置该变量”时写入默认值（`:` + `${VAR:=default}`），不会覆盖已 `export` 的变量。
...
- 不强制覆盖外部已 `export` 的环境变量（仅在未设置时写入默认值）。
...
输入：无（只读取已存在的 `MODELSCOPE_API_TOKEN` 的值作为兜底）。
...
- 若直接运行 `./env_force.sh` 而不是 `source`，会退出并返回退出码 `2`。
...
1. 运行脚本前已 `export` 的环境变量

2. `run_model.sh` 内用 `: "${VAR:=default}"` 设定的默认值

`env_force.sh` 的优先级：

- `env_force.sh` 会强制 `export VAR=...` 覆盖同名变量（少数例外：`MODELSCOPE_API_TOKEN` 只在未设置时保留原值）。
...
若目标是严格复现评测镜像的默认行为，不要 source `env_force.sh`，直接运行 `run_model.sh`（或仅手动覆写确实需要变更的少数变量）。
...
`env_force.sh` 会强制设置一整套变量（覆盖当前 shell 中的同名项）。其中几个与 `run_model.sh`/Dockerfile 不同的点（以当前脚本内容为准）：
...
- 首先检测是否被 source：
  ...
  - 如果 `BASH_SOURCE[0] == $0`，说明是直接执行，会打印提示并 `exit 2`。
...
因为环境变量属于当前 shell 进程。直接运行 `./env_force.sh` 时，变量只在子进程里生效，子进程退出后不会影响当前终端，所以脚本会直接退出并返回码 `2`。

### Q2：已在 venv 安装依赖，为什么 run_model.sh 又装了一遍？

`run_model.sh` 会使用 `python3`/`python`（取决于 PATH），不会自动激活 venv。

可选解决：

- 先手动激活 venv 再运行 `./run_model.sh`

- 或使用 `SKIP_PIP_INSTALL=1`，自行管理依赖版本

### Q3：如何只改一个参数且不被 env_force 覆盖？
...
如果先 `export` 再 `source env_force.sh`，会被强制覆盖回 env_force 的值。
...
`run_model.sh` 固定启动在 `0.0.0.0:8000`。如果 8000 端口已占用，需要先停止占用进程，或自行修改启动命令（例如改 `--port`）并同步调用方（评测固定端口不应修改）。