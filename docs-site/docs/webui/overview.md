...
- 输入：浏览器端输入的文本（以及可选的生成参数、可选 RAG 资料）。
...
这一节按 `webui.py` 的真实实现梳理关键路径，便于排查“为什么 UI 行为和预期不一致”。
...
不会直接影响成绩（因为 WebUI 不参与评测），但在本地复现评测时若通过 WebUI 触发了联网抓取/百度搜索，在断网环境会失败。

建议：断网环境下不要勾选“允许联网抓取 URL / 使用百度搜索”。

### Q3：点击“发送”提示无法连接后端怎么办？
...
如果怀疑后端忽略了参数，可以在后端日志里打印收到的请求字段（或临时在 WebUI 侧打印 payload）来确认。
