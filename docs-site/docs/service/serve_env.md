...
- 如果你只想知道服务是干什么、怎么启动：看 [推理服务（用法与定位）](./serve)。
- 如果你想看实现细节：看 [推理服务（代码详解）](./serve_code)。
- 常见故障与调参经验：看 [推理服务（FAQ）](./serve_faq)。
...
- **作用**：仅用于拼出默认 `MODEL_DIR=./model/$MODEL_ID`；一旦你显式设置 `MODEL_DIR`，`MODEL_ID` 对服务加载不再有直接影响。
...
- **默认值**：`0`（假）
- **作用**：设为真时禁用题型分流，所有题都用 `MAX_NEW_TOKENS`。
...
- **内置关键词特点**：偏向区分 bonus/plus 的“算子类”长题（例如包含“算子/spmv/gemm/triton/tilelang”等）。
...
- **注意**：评测机同步到 Gitee 时可能只保留白名单文件；不要依赖该文件一定存在。
...
> 默认关闭；只建议在你确认环境稳定、并且已经把“基础吞吐/稳定性”调好后再尝试。
...
这些变量要么是通用运行时变量，要么是 `serve.py` 为兼容性主动设置的变量；通常不需要你手动调。
...
- `GET /` 必须快速返回（health）。
- `POST /predict` 请求体必须兼容 `{"prompt":"..."}`，返回必须是 `{"response":"..."}`。
- 端口固定 `8000`（`uvicorn.run(..., port=8000)`）。
