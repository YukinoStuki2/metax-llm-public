...
除以上契约外，`serve.py` 还提供了一些辅助接口（不影响评测）：
...
- 调参/对齐 Dockerfile 与脚本：先读 [推理服务（参数与环境变量）](./serve_env)
- 想理解为什么 batch 模式能提速、为什么有多重回退：读 [推理服务（代码详解）](./serve_code)
- 遇到 health 超时、OOM、tokens/s 异常：读 [推理服务（FAQ）](./serve_faq)
